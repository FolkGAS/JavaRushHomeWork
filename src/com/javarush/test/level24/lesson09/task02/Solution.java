    ...
    public static void sort(List<Stock> list) {
        Collections.sort(list, new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                ...
                comp = ((String) stock1.get("name")).compareTo((String) stock2.get("name"));
                if (comp == 0){
                    SimpleDateFormat df = new SimpleDateFormat("yyyyMMdd");
                    comp = df.format((Date) stock2.get("date")).compareTo(df.format((Date) stock1.get("date")));
                }
                if (comp == 0){
                    if (stock1.containsKey("open") || stock1.get("change") == null || (double)stock1.get("change") == 0)
                        change1 = (double)stock1.get("last") - (double)stock1.get("open");
                    else change1 = (double)stock1.get("change");
                    if (stock2.containsKey("open") || stock2.get("change") == null || (double)stock2.get("change") == 0)
                        change2 = (double)stock2.get("last") - (double)stock2.get("open");
                    else change2 = (double)stock2.get("change");
                    comp = Double.compare (change2, change1);
                }
                return comp;
            }
        });
