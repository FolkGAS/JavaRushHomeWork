...
public class Solution {
    ...
    public String getPartOfString(String string, String threadName) {
        String str;
        try{
        str = string.substring(string.indexOf("\t"), string.lastIndexOf("\t")).trim();
        }catch (Exception exc){
            if (Thread.currentThread().getName().equals(FIRST_THREAD_NAME))
                throw new TooShortStringFirstThreadException(exc);
            else if (Thread.currentThread().getName().equals(SECOND_THREAD_NAME))
                throw new TooShortStringSecondThreadException(exc);
