...
public class Solution {
        public static void main(String[] args) {
            ...
            File output = new File(resultFileAbsolutePath);
            List<File> files = new ArrayList<>();
            makeFilesList(files, path);
            ...
            while (deleteFolders(path)){}
            output.renameTo(new File(output.getParent() + "/allFilesContent.txt"));
        }
    public static void makeFilesList(List<File> files, File folder){
        for (File file : folder.listFiles())
            if (file.isDirectory())
                makeFilesList(files, file);
            else {
                if (file.length() > 50)
                    file.delete();
                else files.add(file);
    ...
    public static boolean deleteFolders(File folder){
